...
class RNN_resource_modeling:
    def __init__(self, trainfile, threshold_cpu_upper, threshold_cpu_lower, threshold_traffic_upper, threshold_traffic_lower, max_scaleup):
        ...
        df.columns = ['1CPU', '1MEMORY', '1IO', '1IO', '1TRAFFIC-RX', '1TRAFFIC-TX', '2CPU', '2MEMORY', '2IO', '2IO', '2TRAFFIC-RX', '2TRAFFIC-TX', '3CPU', '3MEMORY', '3IO', '3IO', '3TRAFFIC-RX', '3TRAFFIC-TX', '4CPU', '4MEMORY', '4IO', '4IO', '4TRAFFIC-RX', '4TRAFFIC-TX', '5CPU', '5MEMORY', '5IO', '5IO', '5TRAFFIC-RX', '5TRAFFIC-TX']
        self.target_names = ['1CPU', '1TRAFFIC-RX', '2CPU', '2TRAFFIC-RX', '3CPU', '3TRAFFIC-RX', '4CPU', '4TRAFFIC-RX', '5CPU', '5TRAFFIC-RX']
    ...
    def train(self):
        ...
        batch_size =512
        sequence_length = 36
        generator = self.batch_generator(batch_size=batch_size, sequence_length=sequence_length)

        x_batch, y_batch = next(generator)

        validation_data = (np.expand_dims(self.x_test_scaled, axis=0), np.expand_dims(self.y_test_scaled, axis=0))
    ...
    def test(self, x_data): # 여기서 받는 x_data는 average_resource
        x_data_edit = np.array(x_data)
        x_data_edit = np.ravel(x_data_edit, order='C')
        self.x_test_scaled = self.x_scaler.transform([x_data_edit])
        x = self.x_test_scaled
        x = np.expand_dims(x, axis=0)
        y_pred = self.model.predict(x)
        y_pred_rescaled = self.y_scaler.inverse_transform(y_pred[0])
        #print("y_pred_rescaled : ", y_pred_rescaled)
        # y_pred_rescaled = self.y_scaler.inverse_transform(y_pred)

        '''
        cpu_pred = y_pred_rescaled[:, 0] 
        print("cpu_pred : ", cpu_pred)
        print("cpu_pred[0] : ", cpu_pred[0])
        traffic_pred = y_pred_rescaled[:, 1]
        print("traffic_pred : ", traffic_pred)
        print("traffic_pred[0] : ", traffic_pred[0])
        '''
        ...
        for i in range(len(self.target_names)): #self.target_names = ['1CPU', '1TRAFFIC-RX', '2CPU', '2TRAFFIC-RX', '3CPU', '3TRAFFIC-RX']
            print("i :", i, "Threshold[i%2] : ", Threshold[i%2])

            if i % 2 == 0:  #0, 2, 4, 6, 8, 10
                cpu_pred = y_pred_rescaled[:, i][0]
                print("i : ", i, "cpu_pred = y_pred_rescaled[:, i][0] : ", cpu_pred)
                server_predicted = self.server_needed_function(cpu_pred, Threshold[i % 2])
                cpu_pred_matrix.append(cpu_pred)
                #print("i : ", i, "cpu_pred_matrix : ", cpu_pred_matrix)
            else:
                traffic_pred = y_pred_rescaled[:, i][0]
                print("i ", i, "traffic_pred = y_pred_rescaled[:, i][0] :", traffic_pred)
                server_predicted = self.server_needed_function(traffic_pred, Threshold[i % 2])
                traffic_pred_matrix.append(traffic_pred)
                #print("i : ", i, "traffic_pred_matrix : ", traffic_pred_matrix)

            Total_server_predicted.append(server_predicted)

        print("cpu_pred_matrix : ", cpu_pred_matrix)
        print("traffic_pred_matrix : ", traffic_pred_matrix)
        print("Total_server_predicted : ", Total_server_predicted)

            #return y_pred_rescaled, server_predicted
        return cpu_pred_matrix, traffic_pred_matrix, Total_server_predicted
    ...
    def plot_comparison_with_scaling(self, start_idx, length=100, train=False):
        ...
        x = x[start_idx:end_idx] #x[0:100]
        #print("x[start_idx:end_idx : ", x)
        print("x.shape = : ", x.shape) # x.shape=(1347,7)   x.shape=(63, 31)
        y_true = y_true[start_idx:end_idx]
        #print("y_true : ", y_true)
        print("y_true.shape = : ", y_true.shape) # y_true.shape = (1347,2)  y_true.shape = (63, 10)

        # Input-signals for the model.
        x = np.expand_dims(x, axis=0)
        print("x_expanded_dims.shape : ", x.shape) # x.shape=(1,1347,7)   x.shape=(1,63,31)
        ...
        y_pred = self.model.predict(x)
        #print("y_pred : ", y_pred)
        # [[[ 0.06137885  0.08103882]              [[[nan, nan, nan, nan, nan, nan]
        #   [ 0.08498137  0.12346219]                [nan, nan, nan, nan, nan, nan]
        #   [ 0.09640671  0.13590807]                [nan, nan, nan, nan, nan, nan]
        #   ...                                      ...
        #   [-0.006999   -0.06299549]                [nan, nan, nan, nan, nan, nan]
        #   [-0.00697549 -0.06286141]                [nan, nan, nan, nan, nan, nan]
        #   [-0.00717813 -0.0633306 ]]]              [nan, nan, nan, nan, nan, nan]]]

        print("y_pred.shape : ", y_pred.shape) # y_pred.shape=(1,1347,2)             y_pred.shape=(1,63,10)

        # The output of the model is between 0 and 1.# Do an inverse map to get it back to the scale # of the original data-set.
        y_pred_rescaled = self.y_scaler.inverse_transform(y_pred[0])
        print("y_pred_rescaled : ", y_pred_rescaled.shape) # y_pred_rescaled.shape = (1347,2)       y_pred_rescaled.shape = (63,10)
        ...
        for signal in range(len(self.target_names)): #self.target_names = ['1CPU', '1TRAFFIC-RX', '2CPU', '2TRAFFIC-RX', '3CPU', '3TRAFFIC-RX']
            ...
            server_predicted = self.server_needed_list(signal_pred, Threshold[signal%2]) #server_list 반환 : sever_predicted[cpu=0], server_predicted[traffic=1]
            ...
            server_needed = self.server_needed_list(signal_true, Threshold[signal%2]) # server_list 반환 : sever_needed[cpu=0], server_needed[traffic=1]
            ...
            plt.plot(t, signal_true, label='true', linewidth=2.0)
            plt.plot(t, signal_pred, label='pred', linewidth=2.0)
...
if __name__ == '__main__':
    ...
    threshold_cpu_upper = 35
    threshold_traffic_upper = 2500
    threshold_cpu_lower = 30
    threshold_traffic_lower = 2000
    waite_time = 5
    RNN_model = RNN_resource_modeling("measurement.csv", threshold_cpu_upper, threshold_cpu_lower, threshold_traffic_upper, threshold_traffic_lower, waite_time)  # CPU,Memory,I/O, Traffic in, Traffic Out for each VNF
    RNN_model.train()
