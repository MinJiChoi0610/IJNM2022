...
import os.path
import predict_resource_RNN_new_algorithm_220331 as predict_resource_RNN_new_algorithm
...
class scaleup_down:
    def __init__(self, prefix, sfc_name, trainfile, scaling_duration, no_of_reading_per_unit_time, waite_time,
                 threshold_cpu_upper, threshold_memory_upper, threshold_IO1_upper, threshold_IO2_upper,
                 threshold_traffic_upper, threshold_traffic_TX_upper, threshold_cpu_lower, threshold_memory_lower,
                 threshold_IO1_lower, threshold_IO2_lower, threshold_traffic_lower, threshold_traffic_TX_lower,
                 max_scaleup):
        self.prefix = prefix
    ...
    def log_scaling_event(self, sum_cpu, avg_cpu, predicted_cpu, sum_traffic, avg_traffic, predicted_traffic, scaled_vnf):
        ...
        self.log_scaling_file.write(str(sum_cpu) + "," + str(avg_cpu) + "," + str(predicted_cpu) + "," + str(sum_traffic) + "," + str(
                avg_traffic) + "," + str(predicted_traffic) + "," + str(scaled_vnf))
        ...
        for tiernum in range(0, no_of_tier):
            ...
            for type in self.resource_type:
                ...
                for i in range(num_vnf_at_each_tier_c[tiernum]):  # for i in range(no_of_vnf_in_tier1):
                    vnf = sfc_info.vnf_instance_ids[tiernum][
                        i]  # t=0 : firewall, t=1:ids, t=2:proxy ....  i=0 : 각 tier당 1번 VNF, i=1 : 각 tier당 2번 VNF, ...
            ...
            Total_resource.append(list(
                resource))  # [[CPU, Memory, Disk, Disk, Tx, Rx],[CPU, Memory, Disk, Disk, Tx, Rx],[CPU, Memory, Disk, Disk, Tx, Rx]]
            # print("Total_resource : ", Total_resource, "\n")
            Total_Avg_resource.append(list(
                Avg_resource))  # 평균 [[CPU, Memory, Disk, Disk, Tx, Rx],[CPU, Memory, Disk, Disk, Tx, Rx],[CPU, Memory, Disk, Disk, Tx, Rx]]
    ...
    def store_measurement_to_train_RNN(self):
        ...
        for d in range(self.scaling_duration):
            ...
            for tiernum in range(no_of_tier):
                Total_max_resource.append(list(
                    max_resource))  # Total_max_resource = [[tier1's resource 6개], [tier2's resource 6개], [tier3's resource 6개]]
    ...
    def test_RNN_scaling(self):
        RNN_model = predict_resource_RNN_new_algorithm.RNN_resource_modeling(self.trainfile, self.threshold_cpu_upper,
                                                                             self.threshold_cpu_lower,
                                                                             self.threshold_traffic_upper,
                                                                             self.threshold_traffic_lower,
                                                                             self.max_scaleup)  # CPU,Memory,I/O, Traffic in, Traffic Out for each VNF
        ...
        for d in range(self.scaling_duration):
            ...
            for t in range(
                    self.no_of_reading_per_unit_time * 3 - 6):  # from 1 ~ 12 총 1 min(한 단계당 5초 * 12번 = 60초) # 12*3-6=30 * 5초 = 150초
            ...
            Copied_Total_max_resource_avg = copy.deepcopy(Total_max_resource_avg)
            #Copied_Total_max_resource_avg.insert(0, d)
            ...
            cpu_pred_matrix, traffic_pred_matrix, Total_server_predicted = RNN_model.test(Copied_Total_max_resource_avg)

            for tiernum in range(no_of_tier):  # CPU와 Traffic 량 예측 : cpu_pred = [tier1'cpu, tier2'cpu, tier3'cpu],
                '''
                print("tiernum : ", tiernum, "[Total_max_resource_avg[tiernum]] : ", [Total_max_resource_avg[tiernum]])
                print("tiernum : ", tiernum, "x_data[tiernum] : ", x_data[tiernum])
                print("tiernum : ", tiernum, "x_data[0][tiernum+1] : ", x_data[0][tiernum+1])

                #cpu_pred[tiernum], traffic_pred[tiernum], Total_server_predicted[tiernum] = RNN_model.test(x_data[0][tiernum+1])
                #cpu_pred[tiernum], traffic_pred[tiernum], Total_server_predicted[tiernum] = RNN_model.test()
                # cpu_pred_matrix, traffic_pred_matrix, Total_server_predicted = RNN_model.test()
                '''

                Copied_Total_max_resource_avg[tiernum][0] = cpu_pred_matrix[tiernum]
                Copied_Total_max_resource_avg[tiernum][4] = traffic_pred_matrix[tiernum]

            print("Total_max_resource_avg : ", Total_max_resource_avg)
            print("Copied_Total_max_resource_avg : ", Copied_Total_max_resource_avg)

            # based on the predicted traffic scaleup or scale down
            starttime = time.perf_counter()

            sfc_info, no_of_tier, num_vnf_at_each_tier_c, scaling_flag_for_each_tier = self.Calculate_ScaleInOut_Value(
                Copied_Total_max_resource_avg)
            scaled_VNF_at_each_tier = self.ActivateScaling(sfc_info, no_of_tier, num_vnf_at_each_tier_c,
            ...
            for tiernum in range(no_of_tier):
                print("Tier : ", tiernum,
                      "log: actual CPU usage, predicted CPU usage, actual traffic, predicted traffic",
                      Total_max_resource_avg[tiernum][0], Copied_Total_max_resource_avg[tiernum][0], Total_max_resource_avg[tiernum][4], Copied_Total_max_resource_avg[tiernum][4])
                # print("Scaling flag for each tier : ", scaling_flag_for_each_tier)
                self.log_scaling_event(Total_max_resource_sum[tiernum][0], Total_max_resource_avg[tiernum][0], Copied_Total_max_resource_avg[tiernum][0],
                                       Total_max_resource_sum[tiernum][4], Total_max_resource_avg[tiernum][4], Copied_Total_max_resource_avg[tiernum][4],
                                       scaled_VNF_at_each_tier[tiernum])
    ...
    def test_threshold_scaling(self):
        output_measurement = open(self.trainfile, 'w')
        RNN_model = predict_resource_RNN_new_algorithm.RNN_resource_modeling("dummy.csv", self.threshold_cpu_upper,
                                                                             self.threshold_cpu_lower,
                                                                             self.threshold_traffic_upper,
                                                                             self.threshold_traffic_lower,
                                                                             self.max_scaleup)  # CPU,Memory,I/O, Traffic in, Traffic Out for each VNF
        ...
        for d in range(self.scaling_duration):  # scaling_duration = 1500
            ...
            for t in range(
                ...
                time1 = time.perf_counter()
                end_time = dt.datetime.now()
                # Sdt.datetime.now(gettz('Asia/Seoul'))
                # end_time = dt.datetime.now(gettz('UTC'))
                start_time = end_time - dt.timedelta(seconds=self.waite_time)
                Total_resource, Total_Avg_resource = self.get_resource_utilization(self.sfc_name, start_time, end_time)
                # print("Total_resource : ", Total_resource)
            ...
            for tiernum in range(no_of_tier):
                print("Tier : ", tiernum,
                      "log: actual CPU usage, predicted CPU usage, scaling VNFs for CPU, actual traffic, predicted traffic, scaling VNFs for Traffic",
                      Total_max_resource_avg[tiernum][0], Total_max_resource_avg[tiernum][4])
                # print("Scaling flag for each tier : ", scaling_flag_for_each_tier)
                self.log_scaling_event(Total_max_resource_sum[tiernum][0], Total_max_resource_avg[tiernum][0], " ",
                                       Total_max_resource_sum[tiernum][4], Total_max_resource_avg[tiernum][4], " ",
                                       scaled_VNF_at_each_tier[tiernum])
                self.log_scaling_file.write(",")
            self.log_scaling_file.write("\n")
        ...
        for tiernum in range(no_of_tier):  # n
            ...
            for k in range(len(self.resource_type)):  # k
                if k == 0 or k == 4:  # In case of CPU or Traffic
                    ...
                    R_Total_Avg_resource[tiernum][k] = 1
                multiplied_R_Total_Avg_resource[tiernum] = multiplied_R_Total_Avg_resource[tiernum] * \
                                                           R_Total_Avg_resource[tiernum][k]
            ...
            if multiplied_R_Total_Avg_resource[tiernum] == 0:  # Scale Out!
                for type in range(len(self.resource_type)):  # resource_type = [CPU, Mem, IO, IO, Traffic-RX, TX]
                    if type == 0 or type == 4:  # In case of CPU or Traffic
                        Total_scale_out_calculation[tiernum][type] = int(
                            int(Total_max_resource_avg[tiernum][type]) / int(upper_Threshold[type]) -
                            num_vnf_at_each_tier_c[tiernum])
                ...
                for type in range(len(self.resource_type)):  # resource_type = [CPU, Mem, IO, IO, Traffic-RX, TX]
                    if type == 0 or type == 4:  # In case of CPU or Traffic
                        Total_scale_in_calculation[tiernum][type] = int(
                            num_vnf_at_each_tier_c[tiernum] - int(Total_max_resource_avg[tiernum][type]) / int(
    ...
    def ActivateScaling(self, sfc_info, no_of_tier, num_vnf_at_each_tier_c, scaling_flag_for_each_tier):
        ...
        for tiernum in range(no_of_tier):
            if scaling_flag_for_each_tier[tiernum] > 0:  # Scale Out
                ...
                print("tierNum :", tiernum, "     scalingup ---- sfc_info.vnf_instance_ids:",
                      sfc_info.vnf_instance_ids[tiernum])
                ...
                if num_vnf_at_each_tier_c[tiernum] + scaling_flag_for_each_tier[tiernum] > 0:
                    ...
                    print("tierNum :", tiernum, "     scalingdown ---- sfc_info.vnf_instance_ids:",
                          sfc_info.vnf_instance_ids[tiernum])
                else:
                    for scaling in range(abs(scaling_flag_for_each_tier[tiernum])):
                        # delete VNF instance code later
...
if __name__ == '__main__':
    ...
    with open('input.json') as f:
        ...
        upper_Threshold = [threshold_cpu_util_upper, threshold_mem_upper, threshold_io1_upper, threshold_io2_upper,
                           threshold_request_per_sec_upper, threshold_traffic_tx_upper]
        lower_Threshold = [threshold_cpu_util_lower, threshold_mem_lower, threshold_io1_lower, threshold_io2_lower,
                           threshold_request_per_sec_lower, threshold_traffic_tx_lower]

        # this is the function for calling the scale up-down constructor.
        scaleup_down = scaleup_down(prefix, sfc_name, "measurement.csv", int(scaling_duration),
                                    int(no_of_reading_per_unit_time), int(waite_time), int(threshold_cpu_util_upper),
                                    int(threshold_mem_upper), int(threshold_io1_upper), int(threshold_io2_upper),
                                    int(threshold_request_per_sec_upper), int(threshold_traffic_tx_upper),
                                    int(threshold_cpu_util_lower), int(threshold_mem_lower), int(threshold_io1_lower),
                                    int(threshold_io2_lower), int(threshold_request_per_sec_lower),
