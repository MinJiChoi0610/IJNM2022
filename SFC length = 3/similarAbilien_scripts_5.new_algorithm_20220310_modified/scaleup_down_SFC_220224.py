...
class scaleup_down:
    ...
    def log_scaling_event(self, cpu, predicted_cpu, traffic, predicted_traffic, scaling_decision):
        # self.log_scaling_file.write(str(traffic) + ","+ str(predicted_traffic[0][0]) + "," + str(scaling_decision) + "\n")
        self.log_scaling_file.write(str(cpu) + "," + str(predicted_cpu[0][0])  + "," + str(traffic) + ","+ str(predicted_traffic[0][0]) + "," + str(scaling_decision))
        ...
        for d in range(self.scaling_duration):
            ...
            for t in range(self.no_of_reading_per_unit_time):
                ...
                for tiernum in range(no_of_tier):
                    ...
                    print("Tier #", tiernum, "max_resource:", max_resource)
                    Total_max_resource.append(list(max_resource))
    ...
    def test_RNN_scaling(self):
        ...
        for d in range(self.scaling_duration):
            ...
            self.log_scaling_event(x_data[0][1], predicted_cpu, x_data[0][2], predicted_memory, x_data[0][5], predicted_traffic, scaling_flag_for_each_tier[tiernum])
    ...
    def test_threshold_scaling(self):
        ...
        for d in range(self.scaling_duration): # scaling_duration = 1483 for wikki.
            ...
            Total_max_resource = []
            for tiernum in range(no_of_tier):
                #Total_max_resource = [[tier1's resource 6개], [tier2's resource 6개], [tier3's resource 6개]]
                Total_max_resource.append(list(max_resource))
            #print("Total_resource_format : ", Total_max_resource)
            ...
            Total_max_resource.insert(0, d)
            Total_x_data = Total_max_resource
            print("Total_x_data : ", Total_x_data)

            server_needed_cpu_for_each_tier = []
            server_needed_traffic_for_each_tier = []
            for tiernum in range(no_of_tier):
                server_needed_cpu_for_each_tier.append(0)
                server_needed_traffic_for_each_tier.append(0)
                #print("Total_x_data[tiernum+1][0] :", Total_x_data[tiernum+1][0])
                #print("Total_x_data[tiernum+1][4] :", Total_x_data[tiernum + 1][4])
                server_needed_cpu_for_each_tier[tiernum] = RNN_model.server_needed_function(Total_x_data[tiernum+1][0], self.threshold_cpu_upper)
                server_needed_traffic_for_each_tier[tiernum] = RNN_model.server_needed_function(Total_x_data[tiernum+1][4], self.threshold_traffic_upper)
            ...
            for tiernum in range(no_of_tier):
                print("Tier : ", tiernum, "log: actual CPU usage, predicted CPU usage, scaling VNFs for CPU, actual traffic, predicted traffic, scaling VNFs for Traffic", Total_x_data[tiernum+1][0], server_needed_cpu_for_each_tier[tiernum], Total_x_data[tiernum+1][4], server_needed_traffic_for_each_tier[tiernum])
                # print("Scaling flag for each tier : ", scaling_flag_for_each_tier)
                self.log_scaling_event(Total_x_data[tiernum+1][0], " " , Total_x_data[tiernum+1][4], " " , scaling_flag_for_each_tier[tiernum])
            ...
            for tiernum in range(no_of_tier):
                ...
                max_resource_str = str(Total_max_resource[tiernum+1])[1:len(str(Total_max_resource[tiernum+1]))-1]
    ...
    def sclaup_down(self):
        ...
        for tiernum in range(no_of_tier):
            ...
            for type in range(len(self.resource_type)):  # resource_type = [CPU, Mem, IO, IO, Traffic-RX, TX]
                if type == 0 or type == 4:  # In case of CPU or Traffic
                    Total_scale_in_calculation[tiernum][type] = int(num_vnf_at_each_tier_c[tiernum] - int(Total_Avg_resource[tiernum][type]) / int(lower_Threshold[type]))
                    if Total_scale_in_calculation[tiernum][type] > 0:
                        Total_scale_in_calculation[tiernum][type] = int(Total_scale_in_calculation[tiernum][type])
                    else:
                        Total_scale_in_calculation[tiernum][type] = 4

                    Total_scale_out_calculation[tiernum][type] = int(int(Total_Avg_resource[tiernum][type]) / int(upper_Threshold[type]) - num_vnf_at_each_tier_c[tiernum])
        ...
        for tiernum in range(no_of_tier):
            ...
            elif scaling_flag_for_each_tier[tiernum] < 0:
